...
def write_compiled_state(fileObject,state):
    ...
    for key in keys:
        ...
        for eachBin in state[key].keys():
            fileObject.write('$'+small_string(eachBin)+'|'+small_string(state[key][eachBin]))
    fileObject.write('\n')
    return True
...
def parse_state_from_str(string):
    state={}
    splitstring=string.split(',')
...
def read_compiled_states(fileObject):
    ...
    for i in range(len(lines)):
        ...
        for j in range(len(singleElements)):
            state[singleElementNames[j]]=singleElements[j]
